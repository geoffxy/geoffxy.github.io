document.addEventListener("DOMContentLoaded", function() {
  const PUB_ENTRY_SELECTOR = ".pub-entry";
  ...
  bindClickHandler(BIBTEX_TOGGLE_SELECTOR, function(event) {
    ...
    const entryEl = event.target.closest(PUB_ENTRY_SELECTOR);
    const wasVisible = toggleDisplayStyle(entryEl.querySelector(BIBTEX_CONTENT_SELECTOR));
    if (wasVisible) {
